...
namespace :db do
  ...
  TEST_DB_NAME = db.path[1..-1]

  desc "Create the database at #{TEST_DB_NAME}"
  task :createtest do
    puts "Creating database #{TEST_DB_NAME} if it doesn't exist..."
    exec("createdb #{TEST_DB_NAME}")
  end

  desc "Drop the database at #{TEST_DB_NAME}"
  task :droptest do
    puts "Dropping database #{TEST_DB_NAME}..."
    exec("dropdb #{TEST_DB_NAME}")
  end
